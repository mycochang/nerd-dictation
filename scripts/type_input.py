...
def type_text(text):
    """
    Simulates keyboard input using ydotool (OPTIMIZED).
    """
    ...
    try:
        ...
        # Type the text with ZERO delay for speed
        # --next-delay 0 makes it nearly atomic
        subprocess.run(["ydotool", "type", "--next-delay", "0", text], env=env, check=True)
